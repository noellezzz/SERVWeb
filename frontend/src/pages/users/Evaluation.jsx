...
export default function Evaluation() {
  ...
  useEffect(() => {
    ...
    } else if (!pushToTalk && !typeToAnswer) {
      setTimeout(() => {
  ...
  const handleNavigation = (direction) => {
    ...
    if (typeToAnswer) {
      setQuestions((prev) => prev.map((q, index) => (index === currentCard ? { ...q, answer: transcript } : q)));
    }
  ...
  return (
    <div className='p-4 flex flex-col items-center'>
      ...
      >
        ...
        {start && <div className='absolute z-50 top-10 right-10 text-sm'>Duration: {duration}</div>}
